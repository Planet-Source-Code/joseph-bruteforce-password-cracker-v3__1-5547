...
If PUFlag = 0 Then
    PUFlag = 1
    a = MsgBox("This is a demonstration of how the permuted word can be used internally in code to check out for possible password combination.Here when ever the permuted word is JOFU the code breaks and the thus you can determine the internal speed. All other external time delay as seen in the other bruteforce attack has been eliminated here", vbInformation, "Bruteforce for PowerUsers")
End If
If PCCode = "JOFU" Then
    Debug.Print PCCode
